...
function sb_print_next_page_link($limit = 0) {
	...
	if ($current < ceil($record_count / $limit)) {
		...
		echo '<a href="'.$url.'">'.__('<img alt="Next Page" style="padding-top:15px;" src="http://sermons.mvchurchofchrist.org/wp-content/plugins/sermon-browser/sb-includes/icons/NextPageButton.png" />', $sermon_domain).'</a>';
	}
}
...
function sb_print_prev_page_link($limit = 0) {
	...
	if ($current > 1) {
		$url = sb_build_url(array('page' => --$current));
		echo '<a href="'.$url.'">'.__('<img alt="Previous Page" style="padding-top:15px;" src="http://sermons.mvchurchofchrist.org/wp-content/plugins/sermon-browser/sb-includes/icons/PreviousPageButton.png" />', $sermon_domain).'</a>';
	}
}
...
HERE;
	...
	if ($filter['filter'] == 'oneclick') {
			...
			<form method="post" id="sermon-filter" action="<?php echo sb_display_url(); ?>">
				<div style="clear:both">
					<table class="sermonbrowser">
						<tr>
							<td class="fieldname"><?php _e('Speaker', $sermon_domain) ?></td>
        ...
        <tr>
                        ...
                        <td colspan="3">&nbsp;</td>
							<td class="field"><input type="submit" class="filter" value="<?php _e('Search &raquo;', $sermon_domain) ?>">			</td>
                            </table>
                            </form>
